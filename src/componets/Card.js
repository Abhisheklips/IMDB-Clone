...
const Card = ({result}) => {
  return (
    ...
    sm:border sm:border-slate-400 group sm:m-2 transition-shadow duration-200'>

   <Link href={`/movie/${result.id}`}>
       <Image src={`https://image.tmdb.org/t/p/original/${result.backdrop_path || result.poster_path}`}
       width={500} height={300} alt="movie image" className='sm:rounded-t-lg 
      ...
      //  placeholder='blur'
       ...
       <div className='p-2'>
        ...
        <AiFillHeart  className='text-red-500 ml-3'/>{result.vote_count}</p>
       </div>
